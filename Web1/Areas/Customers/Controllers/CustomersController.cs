...
namespace Web1.Areas.Customers.Controllers
{
    public class CustomersController : Csla.Web.Mvc.Controller
    {
        ...
        public ActionResult Edit(int id, CustomerEditViewModel customerEditViewModel)
        {
            ...
            if (SaveObject<Customer>(customerEditViewModel.ModelObject, true))
            {
                return RedirectToAction("Index");
            }
            else
        ...
        public async Task<ActionResult> Delete(int? id, bool? saveChangesError = false)
        {
            ...
            var ID = id ?? 0;
            var customer = Customer.GetCustomer(id ?? 0);
            if (customer == null)
            {
                return HttpNotFound();
            }
        ...
        public async Task<ActionResult> DeleteConfirmed(int id)
        {
            ...
            if (SaveObject<Customer>(customerEditViewModel.ModelObject, c => c.Delete(), true))
                return RedirectToAction("Index");
            else
            {
                return View();
